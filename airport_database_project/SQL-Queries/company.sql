-- Изменяем стандартный разделитель с ";" на "//" для корректной обработки ";" внутри процедуры
DELIMITER //

-- Создаем новую хранимую процедуру с именем GetCompaniesWithMinFlights
-- Процедура принимает один входной параметр should_rollback типа BOOLEAN
...
BEGIN
    ...
    HAVING                          -- Фильтруем группы после агрегации
        COUNT(t.id) = (             -- Фильтр: только компании с минимальным числом рейсов
...
DELIMITER ; -- Восстанавливаем стандартный разделитель команд
